...
export default function OrganizedTripsManagement() {
  const { organizedTrips, tripReservations, addOrganizedTrip, updateOrganizedTrip, deleteOrganizedTrip, updateTripReservationStatus } = useAppStore();
            ...
            <div className="space-y-4 pr-4">
              {selectedTrip && getTripReservations(selectedTrip.id).length > 0 ? (
                getTripReservations(selectedTrip.id).map(res => (
                  <div key={res.id} className="bg-slate-50 p-4 rounded-lg border border-slate-200 flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div>
                ...
                <div className="text-center py-8 text-slate-500">Aucune réservation pour ce voyage.</div>
              )}
            </div>
          </ScrollArea>
        </DialogContent>
